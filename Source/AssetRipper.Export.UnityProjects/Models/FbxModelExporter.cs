...
{
	/// <summary>
	/// Exports models to GLB format (FBX alternative) but excludes skinned meshes which are kept in native format.
	/// This exporter handles GameObjects, Components, and LevelGameManagers.
	/// </summary>
	public partial class FbxModelExporter : BinaryAssetExporter
	{
		public override bool TryCreateCollection(IUnityObjectBase asset, TemporaryAssetCollection temporaryFile, [NotNullWhen(true)] out IExportCollection? exportCollection)
		{
			if (SceneHelpers.IsSceneCompatible(asset))
			{
				// Check if this asset contains any skinned meshes
				if (ContainsSkinnedMesh(asset))
				{
					// Has skinned meshes - let it fall through to default YAML exporter
		...
		/// <summary>
		/// Checks if the asset or its hierarchy contains any skinned meshes.
		/// </summary>
		private static bool ContainsSkinnedMesh(IUnityObjectBase asset)
		{
			IGameObject root = asset switch
			{
			...
			// Check the entire hierarchy for skinned mesh renderers
			foreach (IEditorExtension obj in root.FetchHierarchy())
			{
				if (obj is IGameObject go && go.TryGetComponent(out ISkinnedMeshRenderer? _))
				{
					return true;
				}
			}
		...
		{
			return ExportModel(assets, path, false);
		}
		...
		private static void AddGameObjectToScene(SceneBuilder sceneBuilder, BuildParameters parameters, NodeBuilder? parentNode, Transformation parentGlobalTransform, Transformation parentGlobalInverseTransform, ITransform transform)
		{
			...
			// Only process MeshFilter + MeshRenderer (skip SkinnedMeshRenderer)
				...
				&& parameters.TryGetOrMakeMeshData(mesh, out MeshData meshData))
			{
				...
				// Note: SkinnedMeshRenderer is intentionally NOT handled here
				// Those assets should be exported via the native YAML exporter
				else if (gameObject.TryGetComponent(out IRenderer? renderer) && renderer is not ISkinnedMeshRenderer)
				{
					Logger.Warning(LogCategory.Export, $"Renderer type {renderer.GetType()} not supported in {nameof(FbxModelExporter)}");
				}
			}
